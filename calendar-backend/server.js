...
// This part is for Ricky only
// GET endpoint to retrieve all tasks
app.get("/tasks_ricky", async (req, res) => {
    try {
        const { rows } = await pool.query("SELECT * FROM tasks_ricky");
        const formattedTasks = rows.map(task => ({
            ...task,
            date: task.date instanceof Date ? task.date.toISOString().split("T")[0] : task.date
        }));
...
// Get task by ID
app.get("/tasks_ricky/:id", async (req, res) => {
    try {
        const { id } = req.params;
        const { rows } = await pool.query("SELECT * FROM tasks_ricky WHERE id = $1", [id]);
...
// POST endpoint to create a new task
app.post("/tasks_ricky", async (req, res) => {
    try {
        const { date, title, description } = req.body;
        const id = Date.now();
        ...
        await pool.query(
            "INSERT INTO tasks_ricky (id, date, title, description) VALUES ($1, $2, $3, $4)",
            [id, date, title, description || null]
        );
...
// PUT endpoint to update an existing task
app.put("/tasks_ricky/:id", async (req, res) => {
    try {
        const { id } = req.params;
        const { date, title, description } = req.body;

        const query = `UPDATE tasks_ricky SET date = $1, title = $2, description = $3 WHERE id = $4`;
        const values = [date, title, description, id];
...
// DELETE endpoint to remove a task
app.delete("/tasks_ricky/:id", async (req, res) => {
    try {
        const { id } = req.params;
        await pool.query("DELETE FROM tasks_ricky WHERE id = $1", [id]);
        res.json({ message: "Task deleted successfully!" });
    } catch (err) {
        console.error(err.message);
        res.status(500).json({ error: "Internal Server Error" });
